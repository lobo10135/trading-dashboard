...
with col_center:
    ...
    def get_market_data(ticker, name, rb_threshold_pct, stop_offset_pct, stop_diff, tp_offset, hold_weeks):
        ...
        # Berechnungen
        rb_w1 = abs(w1_data['Close'] - w1_data['Open']) # Berechnet Vorwoche
        range_w0 = abs(w0_data['High'] - w0_data['Low'])
        
        # Real Body %: (Real Body Vorwoche / Range aktuelle Woche) * 100
        percent_change = ((rb_w1 / range_w0) * 100) if range_w0 != 0 else 0.0
        ...
        # Stop Buy/Sell: Basierend auf Schlusskurs (W0) und Range der aktuellen Woche (W0)
        stop_value = (w0_data['Close'] + (range_w0 * stop_offset_pct)) if is_bullish else (w0_data['Close'] - (range_w0 * stop_offset_pct))
        ...
        return {
            ...
            "Real Body Vorwoche": format_de(rb_w1),
            "Range aktuelle Woche": format_de(range_w0),
