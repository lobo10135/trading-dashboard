...
# Zentrierung durch Spacer-Spalten
...
with col_center:
    ...
    if st.button("Markt-Analyse starten"):
        res = ["=" * 65, "     INDICES, BONDS & COMMODITIES GAP and Go weekly   ", "=" * 65 + "\n"]
        m = datetime.datetime.now().month
        n = datetime.datetime.now().strftime("%B")
        
        # Logik aus dem Originalskript
        res.append(check_weekend_gap(fetch_data("^GDAXI"), "DAX 40", 0.0, "up", 1800.0, holding_weeks=11))
        if m in [1, 8]: res.append(f"⏭️ EuroStoxx 50: Überprüfung im {n} deaktiviert.\n")
        ...
        if m in [7, 8]: res.append(f"⏭️ Russell 2000 (Down): Überprüfung im {n} deaktiviert.\n")
        else: res.append(check_weekend_gap(rut, "Russell 2000 (Down)", 10.0, "down", 200.0, 240.0, 0))
        
        dji = fetch_data("^DJI")
        if m in [1, 7]: res.append(f"⏭️ DJI (Up): deaktiviert.\n")
        else: res.append(check_weekend_gap(dji, "Dow Jones Industrial (Up)", 0.0, "up", 5000.0, holding_weeks=10))
        if m in [7, 8]: res.append(f"⏭️ DJI (Down): deaktiviert.\n")
        else: res.append(check_weekend_gap(dji, "Dow Jones Industrial (Down)", 0.0, "down", 3800.0, 3000.0, 0))
        
        if m in [2, 3]: res.append(f"⏭️ T-Bond: deaktiviert.\n")
        else: res.append(check_weekend_gap(fetch_data("ZB=F"), "US 30Y T-Bond Future (Up)", 0.0, "up", 11.0, 28.0, 11))
        if m in [2, 3]: res.append(f"⏭️ T-Note: deaktiviert.\n")
        ...
        if m == 9: res.append(f"⏭️ WTI: deaktiviert.\n")
